...
1. [Белый и чёрный списки](#белый-и-чёрный)
2. [Редактор командной строки](#редактор)
3. [Выводить все команды в тесте кликабельными](#кликабельные)
4. [Свой список команд (стратегий)](#свой-список)
5. [Экспорт и импорт настроек](#экспорт-импорт)
6. [Способы запуска](#запуск)
7. [Режимы VPN и Proxy](#vpn-proxy)
    - [Режим https-proxy](#https-proxy)
8. [Разблокировать не только YouTube](#мои-ресурсы)
9. [Точечная маршрутизация сайтов (доменов)](#точечная-маршрутизация)
10. [Раздача интернета с ByeByeDPI](#раздача)
11. [Автозапуск ByeByeDPI при запуске YouTube](#youtube-autostart)
12. [Настройки для работы с AdGuard](#adguard)
13. [Разблокировать свои ресурсы](#мои-ресурсы)

## <a id="белый-и-чёрный">Белый и чёрный списки</a>
...
## <a id="редактор">Редактор командной строки</a>
...
## <a id="кликабельные">Выводить все команды в тесте кликабельными</a>
...
## <a id="свой-список">Свой список команд (стратегий)</a>
...
## <a id="экспорт-импорт">Экспорт и импорт настроек</a>

Чтобы экспортировать или импортировать настройки, зайдите в **_основные настройки ByeByeDPI_** (шестерёнка справа вверху):

- <img src="images/Screenshot_2025-02-25-21-04-04-667_io.github.romanvht.byedpi-edit[1].jpg" width="200">
...
### <a id="https-proxy">Режим https-proxy</a>

По умолчанию ByeByeDPI запускает локальный SOCKS-прокси, но некоторые прокси-клиенты не способны работать с таким протоколом. В таком случае можно активировать **режим https-прокси**.
Чтобы сделать это, необходимо приписать к стратегии аргументы: `-G` или `--http-connect` (данные аргументы идентичны по смыслу).

Вот пример такой стратегии:
`--http-connect -d1 -s0+s -d3+s -s6+s -d9+s -s12+s -d15+s -s20+s -d25+s -s30+s -d35+s -At,r,s -s1 -o1+s -s-1`

## <a id="точечная-маршрутизация">Точечная маршрутизация сайтов (доменов)</a>
...
Чтобы пропускать через ByeByeDPI только определённые домены можно добавить в стратегию аргумент `--hosts` (`-H`) и вписать нужные домены:
...
Скорее всего, вписывать огромный список доменов в поле для ввода стратегии будет неудобно, поэтому можно указать путь, по которому находится текстовый файл с доменами.
...
> Таким же образом можно указать содержимое фейкового пакета.
...
## <a id="раздача">Раздача интернета с ByeByeDPI (локальный прокси)</a>

Допустим у вас есть **старый планшет** и новый телефон.
На телефон установлен ByeByeDPI, подобрана рабочая стратегия. На **старый планшет** ставится прокси-клиент с поддержкой SOCKS5 и возможностью заворачивать через себя установленные приложения (например `nekobox` и аналоги).

Осталось только сделать так, чтобы оба устройства были в одной локальной сети. Можно их подключить к одной точке доступа, можно раздать точку доступа прямо с телефона, в целом это не имеет значения - главное, чтобы устройства находились в одной локальной сети.

Далее в настройках ByeByeDPI необходимо выставить адреса и порт в разделе Proxy на `0.0.0.0` и `1080` (порт можно любой незанятый).

Запускаем ByeByeDPI.

На **старом планшете** с помощью прокси-клиента подключаемся к серверу, вводя порт (из настроек ByeByeDPI) и ip адрес телефона в локальной сети (речь не про `0.0.0.0`), обычно это что-то вроде 192.168.1.30 или какой выдаст вам DHCP.
Для знающих: поставьте статический ip для телефона, если вы будете часто подключаться через него.

Потом в настройках прокси-клиента (на **старом планшете**) добавляем приложения, которое хотим пустить через прокси.
Проверяем.
...
## <a id="мои-ресурсы">Разблокировать свои ресурсы</a>
...
После этого ищите стратегию [аналогичным образом,](#настройка) которым искали стратегию для YouTube.
