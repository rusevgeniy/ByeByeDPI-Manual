...
Чтобы сделать это, необходимо активировать переключатель HTTP прокси или приписать к стратегии аргументы: `-G` или `--http-connect` (данные аргументы идентичны по смыслу).
...
### <a id="host-blacklist"> "Чёрный список" для доменов</a>

В некоторых случаях требуется ограничить работу ByeByeDPI только на каких-то конретных доменах: например, для `kremlin.ru` и `ya.ru`. Чтобы ByeByeDPI не обрабатывал трафик, с этих доменов необходимо в начале стратегии добавить список доменов в отдельную группу: `-H:kremlin.ru ya.ru -An`. Если говорить простыми словами, данная часть означает: если домен `kremlin.ru` или `ya.ru` - ничего не делай. **Важно** не забыть _-An_ после списка.
