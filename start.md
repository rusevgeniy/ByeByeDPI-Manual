...
Всё чаще приходят жалобы на перехват DNS. Для обеспечения своей безопасности и конфиденциальности авторы данного руководства рекомендуют настроить [DNS over TLS](https://ru.wikipedia.org/wiki/DNS_%D0%BF%D0%BE%D0%B2%D0%B5%D1%80%D1%85_TLS) на уровне всей системы (стоит отметить, что такая возможность есть только на устройствах под упралением Android 9+). Данное действие также обеспечит корректную работу ByeByeDPI даже при попытках перехвата DNS.

> [!WARNING]
> В связи с текущей ситуацией с некоторыми DNS-серверами могут возникать проблемы (как, например, с DNS от Cloudflare). Лучше протестировать DNS самому при помощи _DNS Jumper_ и _GRC's DNS Benchmark_.
...
Проверенные DNS-серверы с поддержкой DoT:
