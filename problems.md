...
Для каждой сети нужна своя стратегия. При подключении к другой сети необходимо найти нужные аргументы командной строки, то есть провести [повторный подбор](start.md#настройка). Когда рабочая стратегия для новой сети будет найдена - вы можете [закрепить её и подписать](#features.md#editor) для какой сети данная стратегия.

## <a id="something-is-not-working">Не работает какое-либо приложение или сайт при запущенном ByeByeDPI</a>

Если не работает какое-то определённое приложение, например, Телеграм - используйте [списки](features.md#whitelist-blacklist).

Если не работают какие-то сайты, например, `kremlin.ru` - используйте два браузера: один добавьте в [белый список](features.md#whitelist-blacklist) (в котором будет работать ByeByeDPI), а другой не добавляйте (в нём ByeByeDPI работать не будет).
Если вас не устраивает вариант с двумя браузерами, то используйте методы для [точечной маршрутизации](features.md#split-tunneling).
...
Если проблема не решена - добавьте браузер в [белый список](features.md#whitelist-blacklist) и откройте YouTube там. Если всё работает - проблема в клиенте YouTube.

## <a id="internet-is-lost">На телевизоре (приставке) пропадает интернет при запуске ByeByeDPI</a>

Используйте [белый список](features.md#whitelist-blacklist) или [точечную маршрутизацию](features.md#split-tunneling). Но сначала проверьте, чтобы на приставке вообще был интернет (подключен Wi-Fi или Ethernet-кабель).
...
Чтобы работал Discord вы уже провели [подбор для доменов](#my-services). Но вы не можете подключиться к голосовому серверу.
...
Попробуйте добавить браузер в [белый список](features.md#whitelist-blacklist) и открыть YouTube там. Если всё работает - проблема в клиенте YouTube.
